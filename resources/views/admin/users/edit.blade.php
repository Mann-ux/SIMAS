...
@section('page-description', 'Formulir untuk mengubah informasi guru atau admin yang sudah terdaftar di sistem')
...
<div class="max-w-4xl mx-auto space-y-8">
    ...
    <div>
        <p class="text-[10px] font-bold uppercase tracking-[0.25em] text-gray-400 mb-1">
            Kelola Guru &rsaquo; Edit
        </p>
        <h1 class="text-4xl font-extrabold text-[#00236f] tracking-tight leading-tight">Edit User</h1>
        <p class="text-gray-500 mt-2 text-sm max-w-2xl">Perbarui identitas, akses login, dan peran pengguna dalam sistem akademik SMA.</p>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-[0_12px_40px_rgba(0,35,111,0.06)] overflow-hidden">
        ...
        <div class="px-8 py-6 border-b border-gray-100 flex items-center gap-4">
            <div class="w-12 h-12 bg-[#dce1ff] rounded-full flex items-center justify-center flex-shrink-0">
                <span class="material-symbols-outlined text-[#00236f]" style="font-variation-settings:'FILL' 1;">manage_accounts</span>
            </div>
            <div>
                <h2 class="text-xl font-bold text-[#00236f]">Edit User</h2>
                <p class="text-sm text-gray-500 mt-0.5">Perbarui informasi pengguna: <span class="font-semibold">{{ $user->name }}</span></p>
            </div>
        </div>
        ...
        <form action="{{ route('users.update', $user->id) }}" method="POST">
            ...
            <div class="px-8 py-8 space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    >
                    <p class="text-[11px] text-gray-400 ml-1">Biarkan kosong jika tidak ingin mengubah password.</p>
                    @error('password')
                        <p class="text-xs text-red-500 ml-1">{{ $message }}</p>
                    @enderror
                </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div class="p-5 bg-green-50/60 rounded-xl border-l-4 border-[#006c49]">
            ...
            <p class="text-sm text-[#005236] leading-relaxed">
                Kosongkan kolom password jika tidak ingin mengubahnya. Password lama akan tetap berlaku.
            </p>
        </div>
        <div class="p-5 bg-orange-50/60 rounded-xl border-l-4 border-orange-600">
            ...
            <p class="text-sm text-orange-700 leading-relaxed">
                Peran <span class="font-bold">Wali Kelas</span> dapat ditugaskan ke kelas SMA tertentu setelah data disimpan.
            </p>
        </div>
    </div>
