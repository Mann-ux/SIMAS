...
@section('content')
{{-- ============================================================
     DASHBOARD PENGURUS KELAS — Scholastic Editorial Design System
...
@php $hasRecap = isset($recap_hari_ini) && $recap_hari_ini && array_sum($recap_hari_ini) > 0; @endphp
...
<div class="mb-12">
    <div class="bg-emerald-400 rounded-xl p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between border-l-8 border-emerald-700 shadow-sm gap-4">
        ...
        <a href="{{ route('pengurus.dashboard') }}" class="bg-blue-900 text-white px-6 py-3 rounded-xl font-bold text-sm tracking-wide shadow-md hover:bg-blue-800 transition-all active:scale-95 inline-block text-center flex-shrink-0 whitespace-nowrap">
            LIHAT DETAIL
        </a>
    </div>
</div>
...
<div class="mb-12">
    <div class="bg-amber-400 rounded-xl p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between border-l-8 border-amber-600 shadow-sm gap-4">
        ...
        <a href="{{ route('pengurus.absen.create') }}" class="bg-blue-900 text-white px-6 py-3 rounded-xl font-bold text-sm tracking-wide shadow-md hover:bg-blue-800 transition-all active:scale-95 inline-block text-center flex-shrink-0 whitespace-nowrap">
            INPUT SEKARANG
        </a>
    </div>
</div>
...
<section class="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6 mb-6">
    ...
    <div class="bg-white p-5 md:p-6 rounded-xl shadow-sm relative overflow-hidden group hover:shadow-md transition-all duration-300 flex flex-col">
        ...
        <p class="text-3xl md:text-4xl font-extrabold text-[#00236f] mt-1">{{ $total_siswa }}</p>
    ...
    <div class="bg-white p-5 md:p-6 rounded-xl shadow-sm border-b-4 border-[#006c49] md:border-b-0 md:border-l-4 relative overflow-hidden group hover:shadow-md transition-all duration-300 flex flex-col">
        ...
        <p class="text-3xl md:text-4xl font-extrabold text-[#006c49] mt-1">{{ $recap_hari_ini['Hadir'] ?? 0 }}</p>
        @php
            $rate = $total_siswa > 0 && isset($recap_hari_ini['Hadir']) ? round(($recap_hari_ini['Hadir'] / $total_siswa) * 100, 1) : 0;
        @endphp
    ...
    <div class="bg-white p-5 md:p-6 rounded-xl shadow-sm border-b-4 border-amber-500 md:border-b-0 md:border-l-4 relative overflow-hidden group hover:shadow-md transition-all duration-300 flex flex-col">
        ...
        <p class="text-3xl md:text-4xl font-extrabold text-amber-600 mt-1">{{ ($recap_hari_ini['Sakit'] ?? 0) + ($recap_hari_ini['Izin'] ?? 0) }}</p>
        <p class="text-[10px] md:text-xs font-bold text-orange-500 mt-1 uppercase">BUTUH VERIFIKASI</p>
    </div>

</section>

{{-- ── BOTTOM GRID: Tindakan Cepat + Analisis & Siswa Perlu Perhatian ── --}}
<section class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">

    {{-- ── Tindakan Cepat (Mobile: tampil duluan, Desktop: kanan) ── --}}
    <div class="order-first lg:order-last lg:col-span-5 flex flex-col gap-6">
            ...
            <div class="grid grid-cols-1 gap-3">
                <a href="{{ route('pengurus.absen.create') }}"
                   class="flex items-center gap-3 md:gap-4 bg-[#264191] hover:bg-[#2e4da7] p-3 md:p-4 rounded-xl transition-all active:scale-95 text-left">
                    ...
                    <span class="font-semibold text-sm md:text-base">Input Absensi Harian</span>
                </a>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 md:p-8">
            ...
            <div class="flex justify-between items-end mb-4">
                ...
                @php
                    $pct = $total_siswa > 0 && isset($recap_hari_ini['Hadir'])
                        ? round(($recap_hari_ini['Hadir'] / $total_siswa) * 100)
                        : 0;
                @endphp
                <span class="text-xl md:text-2xl font-extrabold text-[#006c49]">{{ $pct }}%</span>
            </div>
            <div class="w-full bg-slate-100 h-3 rounded-full overflow-hidden">
                <div class="bg-gradient-to-r from-[#00236f] to-[#1e3a8a] h-full rounded-full transition-all duration-1000" style="width: {{ $pct }}%"></div>
            </div>
            <div class="mt-4 flex gap-4">
                <div class="flex items-center gap-2">
                    <div class="w-2 h-2 rounded-full bg-[#00236f]"></div>
                    <span class="text-[10px] font-semibold text-slate-500 uppercase tracking-tighter">Above Target</span>
                </div>
                <div class="flex items-center gap-2">
                    <div class="w-2 h-2 rounded-full bg-slate-200"></div>
                    <span class="text-[10px] font-semibold text-slate-500 uppercase tracking-tighter">Remaining</span>
                </div>
            </div>
            <p class="mt-3 text-xs text-slate-500">
                Rata-rata kehadiran kelas {{ $classroom?->name ?? '' }} hari ini.
            </p>
        </div>
...
<div class="mt-6 w-full h-40 md:h-48 rounded-xl overflow-hidden relative">
