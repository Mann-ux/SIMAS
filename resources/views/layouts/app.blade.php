...
<head>
    ...
    <title>{{ config('app.name', 'SIMAS') }} — {{ isset($title) ? $title : 'Sistem Informasi Manajemen Absensi Sekolah' }}</title>
    <meta name="description" content="SIMAS adalah platform administrasi sekolah premium untuk mengelola kehadiran, kelas, dan data akademik secara efisien.">
    ...
    <style>
        ...
        .sidebar-scroll::-webkit-scrollbar-thumb { background: rgba(144,168,255,0.25); border-radius: 4px; }
...
<div class="flex h-screen overflow-hidden">
    ...
    >
        ...
        <div class="px-8 py-8 flex items-center gap-4">
            ...
            <div class="w-11 h-11 rounded-xl btn-primary-gradient flex items-center justify-center shadow-ambient flex-shrink-0">
                <svg class="w-6 h-6 text-on_primary" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        <nav class="flex-1 px-4 space-y-1 overflow-y-auto sidebar-scroll" aria-label="Menu navigasi">
            ...
            {{-- Section Label --}}
            <p class="px-4 mb-3 text-label-sm font-semibold uppercase tracking-widest text-on_primary_container opacity-60">
                Menu Utama
            </p>
        ...
        <div class="px-4 py-6" style="background-color: rgba(0,0,0,0.15);">
            <form method="POST" action="{{ route('logout') }}">
                ...
                <button
                    id="btn-logout"
                    type="submit"
                    class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-xl w-full
    ...
    <div class="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <header
            id="top-nav"
            class="glass-nav sticky top-0 z-40 flex-shrink-0"
            role="banner"
        >
            <div class="flex items-center justify-between px-8 py-4 gap-4">
                ...
                <div class="flex items-center gap-4 min-w-0">
                    ...
                    <button
                        ...
                        type="button"
                        class="lg:hidden p-2 rounded-xl bg-surface-container-low text-on_surface_variant
                ...
                <div class="flex items-center gap-3 flex-shrink-0">
                    ...
                    <button
                        ...
                        type="button"
                        class="relative p-2.5 rounded-xl bg-surface-container-low text-on_surface_variant
                    ...
                    >
                        ...
                        <span class="absolute top-2 right-2 w-2 h-2 rounded-full" style="background-color: #4b1c00;" aria-hidden="true"></span>
                    </button>

                    {{-- User Avatar (Top Nav) --}}
                    @auth
                    <div
                        class="w-9 h-9 rounded-xl btn-primary-gradient flex items-center justify-center flex-shrink-0 cursor-pointer"
                        title="{{ auth()->user()->name }}"
                        aria-label="Profil pengguna: {{ auth()->user()->name }}"
                    >
        ...
        <main
            id="main-content"
            class="flex-1 overflow-x-hidden overflow-y-auto bg-surface"
            role="main"
        >
            <div class="px-8 py-10 lg:px-12 lg:py-12 xl:px-16 xl:py-16 max-w-screen-2xl mx-auto">
                ...
                @if(session('success'))
                <div
                    class="mb-8 flex items-start gap-3 px-5 py-4 rounded-xl bg-secondary_container"
                    role="alert"
                    aria-live="polite"
                >
                    <svg class="w-5 h-5 mt-0.5 flex-shrink-0 text-on_secondary_container" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                ...
                @if(session('error'))
                <div
                    class="mb-8 flex items-start gap-3 px-5 py-4 rounded-xl"
                    style="background-color: rgba(75, 28, 0, 0.08);"
                    role="alert"
                    aria-live="assertive"
                >
                    <svg class="w-5 h-5 mt-0.5 flex-shrink-0 text-tertiary" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
        ...
        <footer class="flex-shrink-0 bg-surface-container-low" role="contentinfo">
            <div class="px-8 py-4 lg:px-12 flex items-center justify-between gap-4">
                <p class="text-label-sm text-on_surface_variant">
                    &copy; {{ date('Y') }} SIMAS — Sistem Informasi Manajemen Absensi Sekolah
                </p>
...
<div
    id="sidebar-overlay"
    class="fixed inset-0 z-30 bg-primary/40 backdrop-blur-sm hidden lg:hidden"
    aria-hidden="true"
></div>
